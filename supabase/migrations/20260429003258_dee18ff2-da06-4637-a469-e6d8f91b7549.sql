...
BEGIN
  ...
  IF NEW.fiche_type = 'Cornet/Tulipe/Gaufrette' THEN
    ...
    IF coalesce(NEW.extra_data #>> '{cleaning,lavageMachine}', '') NOT IN ('conforme', 'non_conforme')
      OR coalesce(NEW.extra_data #>> '{cleaning,lavageTorchons}', '') NOT IN ('conforme', 'non_conforme')
      OR coalesce(NEW.extra_data #>> '{cleaning,desinfection}', '') NOT IN ('conforme', 'non_conforme')
      OR coalesce(NEW.extra_data #>> '{cleaning,rangementUstensiles}', '') NOT IN ('conforme', 'non_conforme') THEN
      RAISE EXCEPTION 'Tous les contrôles de nettoyage doivent être marqués Conforme ou Non conforme';
    END IF;
