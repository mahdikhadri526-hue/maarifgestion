...
BEGIN
  ...
  -- Désignation/article: obligatoire sauf Panaché (remplacé par matières premières)
  IF NEW.fiche_type <> 'Panaché' AND length(trim(coalesce(NEW.article, ''))) = 0 THEN
    RAISE EXCEPTION 'La désignation est obligatoire';
  END IF;

  -- N° de lot: obligatoire sauf Panaché (chaque matière a son lot)
  IF NEW.fiche_type <> 'Panaché' AND length(trim(coalesce(NEW.lot_number, ''))) = 0 THEN
  ...
  -- DLC obligatoire sauf pour Décoration et Panaché
  IF NEW.fiche_type NOT IN ('Décoration', 'Panaché') AND length(trim(coalesce(NEW.dlc, ''))) = 0 THEN
  ...
  IF length(NEW.fiche_type) > 120
    ...
    OR length(coalesce(NEW.lot_number, '')) > 200
  ...
  IF NEW.fiche_type = 'Panaché' THEN
    ...
    WHERE coalesce((m->>'selected')::boolean, false) IS TRUE;

    IF panache_mat_count = 0 THEN
      RAISE EXCEPTION 'Sélectionnez au moins une matière première';
    END IF;

    SELECT count(*) INTO panache_mat_incomplete
    FROM jsonb_array_elements(NEW.extra_data->'matieresPremieres') AS m
    WHERE coalesce((m->>'selected')::boolean, false) IS TRUE
      AND length(trim(coalesce(m->>'lot', ''))) = 0;

    IF panache_mat_incomplete > 0 THEN
      RAISE EXCEPTION 'Le N° de lot est obligatoire pour chaque matière première sélectionnée';
    END IF;
  END IF;
