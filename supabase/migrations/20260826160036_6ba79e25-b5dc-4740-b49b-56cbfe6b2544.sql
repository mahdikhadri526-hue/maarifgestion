DROP FUNCTION IF EXISTS public.stock_movement_aggregates(uuid);

CREATE FUNCTION public.stock_movement_aggregates(_pdv_id uuid)
RETURNS TABLE(
  product_id text,
  product_name text,
  category text,
...
CREATE INDEX IF NOT EXISTS idx_stock_movements_pdv_product_created
ON public.stock_movements (pdv_id, product_id, created_at DESC);