...
AS $function$
  SELECT CASE
    ...
    WHEN EXISTS (
      ...
      WHERE pr.user_id = _user_id AND lower(pr.email) = 'gestionmaarif1@gmail.com'
    ) THEN (
      EXISTS (
      ...
      OR EXISTS (
        ...
        WHERE up.user_id = _user_id AND pp.permission_key = _permission_key AND pp.allowed = true
      )
    )
    ...
    WHEN EXISTS (
      SELECT 1 FROM public.user_pdvs up WHERE up.user_id = _user_id
    ) THEN EXISTS (
      SELECT 1
      FROM public.user_pdvs up
