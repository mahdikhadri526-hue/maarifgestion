...
BEGIN
  ...
  -- Pour la fiche Suivi perte, lot et DLC ne sont pas requis
  IF NEW.fiche_type <> 'Suivi perte produit et casse matériel'
     AND length(trim(coalesce(NEW.lot_number, ''))) = 0 THEN
  ...
  IF NEW.fiche_type <> 'Décoration'
     AND NEW.fiche_type <> 'Suivi perte produit et casse matériel'
     AND length(trim(coalesce(NEW.dlc, ''))) = 0 THEN
